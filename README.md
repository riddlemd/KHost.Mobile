...
- **Venues** — keep the places you sing, switch the active one from a header chip (or let it **auto-detect the nearest** as you move), and every performance you log is tagged there — so each venue builds its own history and go-to songs. Add one by hand or by **scanning its KaraFun QR code**; the active venue opens its KaraFun catalog in one tap.
