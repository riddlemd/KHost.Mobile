...
<p align="center">
  ...
  <img src="docs/screenshots/my-songs.png" width="200" alt="My Songs — the wishlist as sortable, swipeable cards with album-art covers, a pinned search box, and a floating + button" />
  ...
  <br />
  <img src="docs/screenshots/song-detail.png" width="200" alt="Song detail — the album cover behind the title, per-performance ratings, enjoyment, performance history, and quick links to YouTube, Spotify, and KaraFun" />
...
- **Tonight set list** — build an on-deck set for the venue on its own tab: add songs, drag to reorder, and check each one off as you sing it, with live *“X of Y done”* progress. Tap a row to peek at the song's details. It's kept separate from your wishlist, so a song you sang earlier today stays unchecked until you check it off here. Your place in the list is kept as you switch tabs and come back.
- **Multiple singers, one device** — share the app with the whole table: each singer keeps their own **My Songs** and **Tonight** set, venues are shared, and the whole app **re-tints to the active singer's color**. Switch with a tap on the header **avatar** — no login, no accounts.
- **Venues** — keep the places you sing, switch the active one from a header chip (or let it **auto-detect the nearest** as you move — a venue you pick yourself stays put across restarts until you hand it back to auto-detect), and every performance you log is tagged there — so each venue builds its own history and go-to songs. Add one by hand or by **scanning its KaraFun QR code**; the active venue opens its KaraFun catalog in one tap — or **shows it as a QR code** for someone else to scan and browse the songbook on their own phone.
- **My Songs** — a personal wishlist of songs to sing, as swipeable cards. Add with the floating **+**, search titles and artists from a box that **stays pinned as you scroll**, filter by genre, tags, year or rating, and sort your way — by song, artist, ratings, when you added a song, or when you last sang it. Everything stays put as you switch tabs and come back. One tap on a card queues a song for tonight.
- **Ratings & history** — rate *every* performance (a "how it went" score) plus a separate **enjoyment** rating, jot per-performance notes, and keep a running performance history for each song. Logging defaults to right now, but the date and time are editable, so a sing you forgot at the time still lands on the night it happened. History opens in its own sheet from the song's details (on My Songs *or* Tonight), where a swipe removes an entry and a press-and-hold changes its date. A song's shown star is **confidence-weighted**: it blends the song's own record with your list's overall average, so a single lucky 5 doesn't outrank a solid ten-performance streak — and the more you sing something, the more its star trusts its own history. Optionally **weight recent performances more** (Settings → *Singing & ratings*) so your current form leads.
...
- **Surprise me** — can't decide what to sing? Tap the 🎲 and it picks one for you, with Reroll right there if you want another. Press and hold it to choose how it picks: skip anything you've sung today, lean toward the songs you sing best (by that confidence-weighted star), stick to favourites or to songs you've never sung, and whether to draw from your filters or the whole list.
...
- **Import & export** — export a singer's whole profile (songs *and* performance history) to a file for backup or to move them to another device, export/import your venue list separately, or pull songs straight in from a public **Spotify** or **YouTube Music** playlist link.
...
- **Spelling check** — if the lookup finds no exact match but something one slip away, a ⚠ appears next to the song's name. Tap it and it asks *"Did you mean Bohemian Rhapsody — Queen?"*; take the correction with one tap and the year, genre and cover fill in behind it, or wave it off and it never asks again.
...
- **What leaves the device** — [Privacy, permissions & network use](https://github.com/riddlemd/KHost.Mobile/wiki/Privacy-and-Network) lists every outside service the app can contact and the setting that switches each one off. The [privacy policy](https://github.com/riddlemd/KHost.Mobile/wiki/Privacy-Policy) is also one tap away in the app, from the header menu → **About**.
