...
- **Multiple singers, one device** — share the app with the whole table. Each singer keeps their own **My List** and **Tonight** set, while the **Venues** you sing at are shared. Tap the header **avatar** for a *“Who's singing?”* switch — no login, no accounts — and the whole app **re-tints to that singer's color** so you always know whose phone it is. Manage the roster (add, rename, pick a color, remove) on the **Singers** page in the ⋮ menu; your last singer is remembered for next launch.
