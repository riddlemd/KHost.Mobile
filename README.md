...
<p align="center">
  ...
  <img src="docs/screenshots/my-songs.png" width="200" alt="My Songs — your karaoke wishlist as sortable, swipeable cards with album-art covers; a search box that stays pinned under the header (advanced filters behind a funnel), a floating + button to add a song, and the Tonight tab in the bottom bar" />
...
- **My Songs** — a personal wishlist of songs to sing, as a swipeable card list. Add a song with the floating **+** button, which opens a quick add sheet. A search box **stays pinned under the header as you scroll** and matches a song's title *or* artist, so a new search never means scrolling back to the top; the rest of the filters (genre, tags, year, how-it-went / enjoyment rating) tuck behind a funnel button that opens them in a sheet — and each active one shows as a removable pill. Sort by title, artist, rating, or date added. Your search, filters, sort, and scroll position stick as you switch tabs and come back. One tap on a card adds a song straight to tonight's set.
