...
{
    private static readonly IAtomicFileWriter Writer = new AtomicFileWriter();
    ...
    public async Task An_interrupted_write_leaves_the_previous_good_file_intact()
    {
        ...
        await Writer.WriteAsync(path, s => s.WriteAsync(System.Text.Encoding.UTF8.GetBytes("original")).AsTask());
        var original = await File.ReadAllTextAsync(path);

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            Writer.WriteAsync(path, async s =>
            {
    ...
    public void Quarantine_when_the_target_doesnt_exist_is_a_no_op()
    {
        ...
        Writer.Quarantine(path);   // nothing to move — must not throw or create a .corrupt file
