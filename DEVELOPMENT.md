...
**Shoot against sample data, never a real library — `docs/screenshots/` is public and the README renders it.** The venue detail sheet prints a venue's **KaraFun Id** in the clear, so a grid shot on the phone you actually use publishes a venue you actually go to, along with whatever performance history is attached to it. This is not hypothetical: the July 2026 re-shoot carried a live venue ID for about four weeks, across three releases, before anyone noticed. Seed the sample venue (`The Mint`, KaraFun Id `012345`) and a sample singer first, and **open every PNG and look at it** before committing — the status bar and header chip give away as much as the content does.
...
The ordering itself lives in `SongEnricher` (Infrastructure), not in the page that triggers a lookup: it returns what to apply and the caller writes it, so a lookup abandoned when a sheet closes can't leave a half-filled song behind, and the rules above are testable without a component. `ImportExport` deliberately runs a narrower metadata-only pass and does not share it.
...
The draw rules live in `Infrastructure/Services/SurprisePicker.cs`, pure and deterministic given a roll value, so the narrowing and the star weighting are unit-tested without a UI or an RNG. Two things worth keeping: a restriction that would empty the pool is **skipped rather than honoured** (a one-tap picker has nowhere to explain "no matches"), and every song keeps a small weight floor so a run of bad nights can't make one undrawable. `press-hold.js` owns the pointer sequence rather than using `@onclick` plus a timer, because the WebView fires its own long-press callout early and still delivers a click afterwards — which would run the hold *and* then roll.

**Year bounds — native `<select>` per end.** Each end of the My Songs year filter is a plain dropdown, alongside the dual-handle slider that drives the same two values. It replaced a bespoke grid-picker sheet: the grid packed a 65-year span into 13 rows where a list needed 65, but a sheet that opens *from* a sheet to set one number reads as an unfamiliar detour, and the OS wheel is the control people already know — the same reason the sort field and genre picker are native selects.

Each dropdown lists **only years actually present in the library** (`_libraryYears`, off the same pass that computes the bounds) **and only those its own end may take** — the earliest stops at the latest, the latest starts at the earliest. So every entry is a pick that works, and none can produce an empty result set. The grid showed the out-of-range years disabled instead; a native select has no equivalent of a greyed tile you can see but not reach, and a long dead block at one end of the wheel is just noise. Widening stays reachable from either end: lowering the earliest re-opens the years below it in the latest, and vice versa.

The one thing a dropdown needs that the grid didn't: the slider steps through *every* year in the span, so it can land on one no song uses. A value with no matching `<option>` doesn't render blank — the select silently falls back to displaying its first option — so `YearOptions` merges the current bound into the list when it isn't a library year.

The slider keeps one wart the dropdowns don't have, and always has: because it steps through every year, dragging **both** handles onto a year no song uses filters the list to nothing. Fixing that means snapping the slider to `_libraryYears`, which turns it into an index-based control.

**Venue auto-detect — three things stop it re-picking the last venue.** Auto-detect is on by default and re-checks every 5 minutes against a 75 m radius, so anything that makes it answer wrongly answers wrongly *repeatedly*, and to the user it reads as the venue resetting at random.

The first rule is that **a bad fix is worse than no fix**: `VenueLocator` treats a null fix as "leave the active venue alone", so `MauiLocationProvider` returns null rather than passing along something it doesn't trust. It discards a fix older than **2 minutes** or with accuracy worse than **500 m**. Both guards exist because the last-known-location fallback is not a rare path — indoors, the 10-second medium-accuracy request routinely times out — and the last place a phone held a lock is, for this app, the last venue you sang at. Handing that back re-selects that venue from your couch. The accuracy floor covers the same failure from the other side: a cell/wifi fix reports a neighbourhood centroid that can land inside a venue's radius by luck.

The second is that **the pin is re-read after the device call, not just before it**. The fix takes seconds, and a manual pick routinely lands inside that window; without the second check the stale answer both moves the venue and clears the pin, leaving every later re-check free to move it again.

The third is that **the pin is persisted**. `IAppSession` is otherwise in-memory, but Android kills a backgrounded app freely, and a pin that died with the process meant every relaunch came back on auto-detect. `AppSession.SetActiveVenue` writes the id and the pin through to `IAppSettings` (mirroring `LastActiveSingerId`) and `MainLayout`'s bootstrap reads them back — unconditionally, not gated on "the venue changed", because the Auto/Pinned flip moves no venue and is exactly the state that has to survive. An empty id means no venue; the pin flag is what tells a deliberate *"not at a venue"* apart from a fresh install, and the bootstrap clears a pointer whose venue has since been deleted rather than leaving the session pinned to nothing. The write-through hangs off an **optional** constructor parameter so tests can `new AppSession()` bare and stay in memory — which makes "DI actually supplied it" invisible at compile time, so `ProjectTests` resolves the real graph and asserts the write lands.
