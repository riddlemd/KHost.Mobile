...
- **The Apple heads pin an exact Xcode major.minor**, and `Directory.Build.props` turns that check off (`ValidateXcodeVersion=false`) so they still compile on whatever Xcode is installed. It is off because the check fails *before* the compiler, so a Mac that moves ahead stops compiling those heads entirely — which is how two real errors sat unseen. Treat an Apple head built here as a compile check only; for something shippable, install the Xcode the workload names and build that head with `DEVELOPER_DIR=/Applications/Xcode-<version>.app/Contents/Developer`.
