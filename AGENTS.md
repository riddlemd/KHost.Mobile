...
- **The Apple heads pin an exact Xcode major.minor** and refuse any other, naming the one they want. A Mac on a newer Xcode fails both Apple heads before the compiler runs — which also means neither has been *compiled* since, so real errors can be sitting behind it. Build them with `DEVELOPER_DIR=/Applications/Xcode-<version>.app/Contents/Developer` pointing at the matching Xcode; `-p:ValidateXcodeVersion=false` gets past the check to see whether the code compiles, but not to produce anything shippable.
