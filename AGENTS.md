...
- `Models/Singer.cs` (id, name, `Color`, optional emoji `Glyph`, `Order`; `Initial` + `Avatar` derived — `Avatar` is the glyph when set, else the name's first letter) + `Models/SingerColors.cs` (8-color picker) + `Models/SingerGlyphs.cs` (the emoji-avatar picker set; the edit sheet also offers a "use the letter" option that clears the glyph). `Services/ISingerStore.cs` + `JsonFileSingerStore.cs` — the roster (`singers.json`). `EnsureSeededAsync` creates a default "Me" on first run and **migrates the legacy single-user `song-list.json` / `tonight.json` into it**.
- The **active singer** is an ephemeral pointer on `IAppSession` (`ActiveSingerId` / `SetActiveSinger` / `ActiveSingerChanged`), remembered across launches via `IAppSettings.LastActiveSingerId`. `MainLayout` gates the whole shell on a bootstrap that seeds + resolves the active singer before any personal page loads.
- The **song-list and tonight stores are namespaced per singer**: they take an optional `IAppSession`, read/write `song-list-{id}.json` / `tonight-{id}.json` for the active singer (see `SingerDataFiles`), reload on `ActiveSingerChanged`, and fall back to the legacy file when no session is wired (the integration-test path). Per-singer files also give per-singer sung history "for free" — a shared venue's "your history here" is already scoped to the active singer.
- **My Songs remembers each singer's own view** — `IAppSession.MySongsViewFor(singerId)` holds one filter/sort/paging state per singer, and `scroll.js` is keyed `mysongs:{singerId}`, so switching singers restores that person's filters, sort, paged height and scroll position. `MySongs.RefreshAsync` detects a singer switch (the store's `Changed` fires as the file swaps under it), stashes the outgoing view, and re-arms the scroll restore for the incoming one.
- **Row gestures are one JS module** — `wwwroot/js/swipe.js` owns tap / press-and-hold / swipe-left for the song, venue and singer lists off a single pointer state machine, so the three gestures can't race each other. `options` names the per-list `idAttr` + `[JSInvokable]` methods; `holdMethod` opts a list into press-and-hold (500 ms, cancelled by any movement past the tap slop or by a drag starting) and `swipeEnabled: false` opts out of swipe-to-remove (the Singers list — a singer is removed from its edit sheet). A hold that fires suppresses the tap that would otherwise land on pointerup. **Press-and-hold sets the active venue / singer**, replacing the old radio dots on both pages; `Services/IHaptics.cs` + `MauiHaptics.cs` tick the platform long-press haptic to confirm it (named `IHaptics`, not `IHapticFeedback`, to avoid MAUI Essentials' same-named interface). Because no assistive-tech gesture maps to a long press, both pages keep a reachable equivalent in their sheet — Venues' *Active* toggle and the singer sheet's *Switch to this singer*.
- **Icon picker is shared** — `Components/IconPicker.razor` is the collapsible color-+-emoji picker (a tappable preview that opens a panel of an optional color row + a glyph grid with an optional "use the letter" tile). The singer editor uses it with a color palette + letter fallback (a colored avatar preview); the venue editor reuses it glyph-only (a surface tile). Both bind `Glyph` (and the singer binds `Color`); it collapses by default and after a glyph pick.
