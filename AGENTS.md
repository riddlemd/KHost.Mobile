...
| `KHost.Mobile.Infrastructure` | Abstractions | The MAUI-free implementations, foldered by kind so `Services/` doesn't become a grab-bag: `Services/` = the JSON stores, `AppSession`, `AlbumArtCache` and the pure logic they support (`RatingScore`, `SurprisePicker`, `AtomicFile`); `Serialization/` = the six `JsonSerializerContext` partials; `Search/` = the KaraFun/Spotify/YouTube link builders and URL parser; `Models/` = parameter records (`RatingConfig`, `RatingContext`, `SurpriseOptions`); `Diagnostics/` = `LoggingHttpMessageHandler`. Anything touching `Microsoft.Maui.*` does **not** belong here. |
