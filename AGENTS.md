...
- **`JsonFileLyricsCache` deliberately does NOT derive from it** — it keeps a `Dictionary<string, LyricsCacheEntry>` in memory while persisting a list, so the base's list-shaped cache doesn't fit. It is the one store still calling `AtomicFile` directly. Generalizing the base to cover it would cost more than the duplication does.
