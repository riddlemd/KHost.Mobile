...
public class MainActivity : MauiAppCompatActivity
{
    protected override void OnCreate(Bundle? savedInstanceState)
    {
        ...
        // Android 15+ / API 36 forces edge-to-edge, and Android's WebView never populates CSS
        // env(safe-area-inset-*) for the system bars — so publish the measured insets to ISafeAreaInsets and let
        // MainLayout write them into the --kh-inset-* CSS variables once its DOM exists. The listener re-fires
        // whenever the insets change, which covers rotation.
        var window = Window!;
        ...
        // Route the hardware/gesture back button through the app's overlay registry, so back closes an open
        // sheet/menu instead of minimizing. The callback stays enabled and decides per-press.
    ...
    // time — the MAUI service provider isn't guaranteed built at OnCreate. Takes the SystemBars ∪ DisplayCutout
    // union: on a punch-hole device the cutout reaches deeper than the status-bar inset (Pixel 7: 136px vs 74px)
    // and the system clock/icons sit clear of the cutout, so only the union keeps header content out from under them.
