...
public interface IAppSession
{
    ...
    /// <summary>
    /// The My Songs list's filter + sort + scroll state <em>for a given singer</em>. Lives here, not on the page,
    /// because the page component is disposed on navigation and its own fields would reset. One instance per singer
    /// id, created on first ask; a null <paramref name="singerId"/> returns a shared fallback (the session-less /
    /// pre-seed path). Never persisted.
    /// </summary>
    ...
    /// <summary>
    /// The venue the singer is "at" right now — performances are stamped with it. Ephemeral like the rest of the
    /// session: re-resolved each launch (manually or by geolocation) and never persisted. Null means "not at a saved
    /// venue", and performances then log untagged. Set via <see cref="SetActiveVenue"/> so
    /// <see cref="ActiveVenueChanged"/> can fire.
    /// </summary>
    ...
    /// changes; the pin state always updates.</summary>
    ...
    /// <summary>
    /// The singer whose personal lists the app is currently showing — this ephemeral pointer selects whose data the
    /// per-singer stores read and write. Re-resolved each launch (from <see cref="IAppSettings.LastActiveSingerId"/>
    /// when still valid, else the first singer) and never persisted here. The bootstrap sets it before any personal
    /// page loads, so it is expected non-null in the UI. Set via <see cref="SetActiveSinger"/> so
    /// <see cref="ActiveSingerChanged"/> can fire.
    /// </summary>
    ...
    /// re-points it). The per-singer stores listen to reload their file.</summary>
    event EventHandler? ActiveSingerChanged;

    /// <summary>The venue whose detail the first-run tour wants open (or null to close it). Set only by the
    /// tutorial; the Venues page listens via <see cref="TutorialVenueDetailChanged"/> and reflects it. Purely a
    /// transient tour affordance — never persisted.</summary>
