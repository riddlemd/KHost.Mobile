...
public sealed class Singer
{
    ...
    /// <summary>The only required field — the singer's display name ("Mike", "Sam").</summary>
    public string Name { get; set; } = string.Empty;

    /// <summary>The singer's accent color (hex), chosen from <see cref="SingerColors"/>. Defaults to the brand
    /// violet.</summary>
    public string Color { get; set; } = SingerColors.Default;

    /// <summary>An optional emoji used for the avatar instead of the name's first letter, from the curated
    /// <see cref="SingerGlyphs"/> set. Null (the default) means "use the first letter". Raw emoji string.</summary>
    public string? Glyph { get; set; }

    /// <summary>Display order. Assigned on add (append to the end); a lower number sorts first.</summary>
